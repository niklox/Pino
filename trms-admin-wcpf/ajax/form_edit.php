...
function defaultAction($formid){
	...
	if( $formid ){
		...
		print	"<input class=\"std_input\"  type=\"text\" title=\"Formname\" name=\"selectedform\" id=\"selectedform\" value=\"". $form->getName(). "\"/>";
		print	"<input type=\"hidden\" name=\"formid\" id=\"formid\" value=\"". $form->getID(). "\"/>";
		print	"<select class=\"std_select\" title=\"Formtype\" id=\"formtype\" name=\"formtype\">\n" .
		...
		print	"<input title=\"Formaction\" class=\"std_input\"  type=\"text\" placeholder=\"Form action\"  name=\"formaction\" id=\"formaction\" placeholder=\"action\" value=\"". $form->getAction()."\" />\n";
		print	"<input title=\"Formrecipient\" class=\"std_input\" type=\"text\" placeholder=\"Email  recipient/s\" name=\"formrecipient\" id=\"formrecipient\" value=\"". $form->getRecipient()."\" size=\"42\"/>\n";
		
		print	"<select class=\"std_select\" title=\"Formstatus\"  id=\"formstatus\" name=\"formstatus\">\n" .
		...
		print	"<input class=\"std_button\" type=\"button\" id=\"deleteform\" value=\"" . MTextGet("deleteform") . "\"/> ";
		print	"<input class=\"std_button\" type=\"button\" id=\"copyform\" value=\"" . MTextGet("copyform") . "\"/>";
			
	}else{
		
		print	"<input class=\"std_input\" placeholder=\"Form name\" type=\"text\" name=\"selectedform\" id=\"selectedform\" value=\"". $formname. "\"/>";
		print	"<input type=\"hidden\" name=\"formid\" id=\"formid\" value=\"0\"/>";
		print	"<select class=\"std_select\" id=\"formtype\" name=\"formtype\">\n" .
				"<option value=\"0\">Standard form</option>\n" .
				"<option value=\"1\">Quiz form</option>\n" .
				"<option value=\"2\">Global Vote</option>\n" .
				"</select>\n";
		
		print	"<input class=\"std_input\" type=\"text\" placeholder=\"Form action\" name=\"formaction\" id=\"formaction\" value=\"\" /> \n";
		print	"<input class=\"std_input\" type=\"text\" placeholder=\"Email  recipient/s\" name=\"formrecipient\" id=\"formrecipient\" value=\"\" />\n";
		
		print	"<select class=\"std_select\" id=\"formstatus\" name=\"formstatus\">\n" .
	...
	if( $formid ){
		...
		print	"<input class=\"std_button\" type=\"button\" id=\"createnewinput\" value=\"Create new input\" /> ";
		...
		print	"<select class=\"std_select\" id=\"forminputtype\">\n" .
				"<option value=\"\">Select input type to create</option>\n" .
...
function editFormInput($forminputid){
	...
	if($forminputid > 0){
		...
		print	"<div id=\"editforminput_header\">". MTextGet(FormInputTypeGetName($forminput->getTypeID())) . " " . $forminputid . " <span id=\"close_btn\">x</span> </div>" .
		...
		if($forminput->getTypeID() == 6){
		print	"Selected índex: <select id=\"selectedindex\">";
		
		for($i=0; $i<50; $i++){
		print	"<option value=\"" . $i . "\" ".($forminput->getCols() == $i ? " selected": "" ). ">" . $i . "</option>";
		}
		print	"</select><br/><br/>";
		}
		...
		if($viewinput == false){
			...
			while($options = FormInputOptionGetNext($options)){
					print	'<input type="text" class="forminputoption_text" id="'. $options->getOptionName() . '" value="' . $options->getLabel() . '"/> &nbsp;<span class="tbtn"><a title="Edit MText for label" href="/trms-admin/mtext.php?action=edit&textid='.$options->getTextID().'&textcatid='.MTextGetCategoryid($options->getTextID(), TermosGetCurrentLanguage()).'" target="_new">T</a></span> ID: '. $options->getOptionName() .'<br/>';
			}
		}else{
		...
		print	"</div>";
		print	"<br/><input type=\"button\" id=\"deleteforminput\" value=\"Delete this forminput\" /> <input type=\"button\" id=\"viewforminput\" value=\"View this forminput\" /><br/><br/>";
		print	"Forminputid: " . $forminputid;
		print	"</div>";
		print	"</div>";
	}
}
...
function saveFormInput($forminputid, $formid){
	...
	if(isset($_REQUEST["selectedindex"]))$selectedindex = $_REQUEST["selectedindex"]; else $selectedindex = 0; 
	...
	if($forminputid > 0){
		...
		$forminput->setCols($selectedindex);
