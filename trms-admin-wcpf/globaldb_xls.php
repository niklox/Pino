...
if(isset($_REQUEST["showtopics"]))$showtopics = $_REQUEST["showtopics"]; else $showtopics = 0;
...
// Start buffering
 ...
 print 	'<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">'.
  ...
  if($showtopics > 0){
  	$activetopics = array();
  	$activetopicsname = array();
  	$counter = 0;
  	$topics = TopicGetAllByStatus(1);
  	while($topics = TopicGetNext($topics)){
  		$activetopics[$counter] = $topics->getID();
  		$activetopicsname[$counter] = $topics->getTopic();
  	 	$counter++;
  	} 
  } 
  
  $nooftopics = count($activetopics);
  ...
  if($contactemail > 0) 	{print '<td>Contact email</td>'; $leadingcells++;}
  if($contacttel > 0) 		{print '<td>Contact telephone</td>'; $leadingcells++;}
  if($telephone > 0) 		{print '<td>Telephone</td>'; $leadingcells++;}
  if($email > 0 || $contactemails > 0) 			{print '<td>E-mail</td>';}
  
  if($showtopics > 0){
  	
  	for($x = 0; $x < $nooftopics; $x++) {
    	print '<td>' . $activetopicsname[$x] . '</td>';
    }
  ...
  while($orgs = OrganisationGetNext($orgs)){
	...
	if($students > 0) print '<td>'.$orgs->getExternalID().'</td>';
  	...
  	if($showtopics > 0){
  		for($x = 0; $x < $nooftopics; $x++) {
    		print '<td>' . (OrganisationHasTopic($activetopics[$x], $orgs->getID()) > 0 ?"1":"") . '</td>';
    	}
  	}
