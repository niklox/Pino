...
function defaultAction(){
	...
	$sql = "SELECT FormItemAnswerText, FormItemAnswerFormAnswerID FROM FormItemAnswers WHERE FormItemAnswerFormItemID = 1048 ORDER BY FormItemAnswerFormAnswerID";

	$result = mysqli_query($dbcnx, $sql);

	while ($row = mysqli_fetch_array($result)) {

		if ($formanswer = FormAnswerGet( $row[1] ))
		//printf("No answer FormAnswerID: %s<br/>", $row[1]); 
		continue;  
		else
		printf("Votes: %s  FormAnswerID: %s<br/>", $row[0], $row[1]);  
	}
