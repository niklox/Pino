...
function defaultAction($formid){
	...
	$formanswer = FormAnswerSearch($startdate, $enddate, $countryid, $votetype, $formid);
	...
	while($forminput = FormInputGetNext($forminput)){
		...
		if($forminput->getTypeID() == 1 || $forminput->getTypeID() == 2){
			print "<td>" . FormInputGetQuestion( $forminput->getID(), $forminput->getTypeID() ) ."</td>";
		}
		else if($forminput->getTypeID() == 4){
			print "<td>" . $forminput->getQuestion() ."</td>";
		}
		else if($forminput->getTypeID() == 9){
			print "<td>" . $forminput->getTitle() ."</td>";
		}		
	...
	while($formanswer = FormAnswerGetNext($formanswer)){
		print	"<tr>\n<td>" .$formanswer->getDateTime(). "</td>";

		$forminput = FormInputGetAllForForm($formanswer->getFormID());
		while($forminput = FormInputGetNext($forminput)){

			if($forminput->getTypeID() == 1 || $forminput->getTypeID() == 2){
			print "<td>" . FormInputGetAnswer($formanswer->getID(), $forminput->getID(), $forminput->getTypeID()). "</td>";
			}
			else if($forminput->getTypeID() == 4){
				print "<td>" . FormInputGetAnswer($formanswer->getID(), $forminput->getID(), $forminput->getTypeID()). "</td>";
			}
